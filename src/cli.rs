...
pub struct Cli {
    /// Path to config file (default: `$XDG_CONFIG_HOME/ciri/config.kdl`).
    ///
    /// This can also be set with the `CIRI_CONFIG` environment variable. If both are set, the
...
pub enum Msg {
    ...
    Output {
        /// Output name.
        ///
        /// Run `ciri msg outputs` to see the output names.
        #[arg()]
        output: String,
